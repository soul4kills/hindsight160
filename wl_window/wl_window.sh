...
case "$1" in
    ...
    *)         echo "Usage: $0 {start|stop|status|install|uninstall|manage|install_cron|activate_cron|deactivate_cron}" ;;
esac
#!/bin/sh
...
install_script() {
    ...
    cru a "$JOB_START" "$START_MM $START_HH * * * $SCRIPT start"
    cru a "$JOB_STOP"  "$END_MM $END_HH * * * $SCRIPT stop"
    echo "[+] Cron jobs installed: start=${START_HH}:${START_MM} stop=${END_HH}:${END_MM}"
...
uninstall_script() {
    ...
    # 1. Remove iptables rules and flush conntrack
    remove_block
    ...
    # 4. Delegate webui teardown to the install script if present
    INSTALL_SCRIPT="$ADDON_DIR/wl_window_install.sh"
    if [ -f "$INSTALL_SCRIPT" ]; then
        sh "$INSTALL_SCRIPT" uninstall
    else
    ...
    # 5. Clean up all wlw_* keys from shared custom_settings.txt
    if [ -f "$SETTINGS" ]; then
        sed -i '/^wlw_/d' "$SETTINGS"
        sed -i '/# WLW_STATUS_START/,/# WLW_STATUS_END/d' "$SETTINGS"
        echo "[-] Settings removed from $SETTINGS."
...
case "$1" in
    ...
    uninstall) uninstall_script ;;
    manage)    manage_list "$2" "$3" "$4" ;;
    *)         echo "Usage: $0 {start|stop|status|install|uninstall|manage}" ;;
esac
